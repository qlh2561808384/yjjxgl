...
<div class="container_fluid">
    <div class="welcomeInfo">
        <i class="ico_idx ico_1"></i><span id="username"></span>，下午好！欢迎使用非税综合管理平台
        &emsp;<i class="ico_idx ico_2"></i>现在是<span id="nowdate"></span>&emsp;您上次登录时间： <span id="lastlogin"></span>
    </div>
    ...
    $(function() {
        ...
        var timer = setInterval(function(){
            if($('#username').text() == ""){
                ...
                $('#username').html(loginUser.username); // 欢迎语
                $('#lastlogin').text(new Date(loginUser.lastLoginTime).toLocaleString().replace(/:\d{1,2}$/,' ')); // 显示用户上次登录时间
                if(userwork.iswork){
