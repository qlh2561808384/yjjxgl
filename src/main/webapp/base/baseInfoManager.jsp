...
<script type="text
				 ...
				 inputs: [//搜索栏表单参数
					 {
						 ...
						 id:'nodenum',
						 checkType:'radio',
						 url : 'pro/getStepcodeTree.do?userKey='+userKey,
						 onAckCallback:function(nodes){
							 if(nodes.length>0){
	...
	function editReport() {
		...
		if (dataArr.length != 1) {
			...
			if(dataArr[0].ISOPEN == 0 && time < dataArr[0].DEADLINEDATE){
				...
				var edit_filename = "<%=ConfigureParser.getPropert("JXGZID")%>";
    ...
    function submitBaseInfo() {
		...
		if (dataArr.length == 0) {
			...
			if(dataArr[0].ISOPEN == 0 && time < dataArr[0].DEADLINEDATE){
				if(dataArr[0].STEPCODE == 1){
					if (null == dataArr[0].XMGK || null == dataArr[0].XMZXQK || null == dataArr[0].CZWTYJ){
						$.dalert({
							text:"填写完整报表之后才能提交哦！",
							icon:7
						});
