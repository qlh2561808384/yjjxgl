$(document).ready(function(){
	$(".treeSel").attr("readonly","readonly");
	$(".ztreeBox").each(function(k){
		k++;
		var ztreeBox = $(this);
		var zNodes_r = [];
		var idVal = 0, pIdVal = 0, openVal = false, nocheckVal = false;
		var sel = ztreeBox.find(".treeSel");
		var treedemo = ztreeBox.find(".treeDemo_check");
		treedemo.attr("id", "treeDemo_check" + k);
		...
		var setting_r = {
			...
			callback: {
				beforeClick: function(treeId, treeNode){
					var zTree = $.fn.zTree.getZTreeObj("treeDemo_check" + k);
